...
describe('SSL/TLS Analyzer Tool', () => {
  ...
  test('handles scan timeout', async () => {
    ...
    await expect(analyzeSslTls('https://example.com', 100, 1)).rejects.toThrow(
      'SSL scan timed out'
    );
  });
  ...
  test('handles API errors correctly', async () => {
    ...
    await expect(analyzeSslTls('https://example.com')).rejects.toThrow(
      'SSL Labs API Error: Invalid hostname'
    );
  });
  ...
  test('handles invalid responses', async () => {
    ...
    await expect(analyzeSslTls('https://example.com')).rejects.toThrow(
      'Invalid response from SSL Labs API'
    );
  });
});
